...
public class EmptyBackGroundTile {
	...
	LevelEditor editor;
	public int index;
	public String selectedLayer = "background";
	
	public EmptyBackGroundTile(int x, int y, MouseHandler mouse, LevelEditor editor){
		this.x = x;
		this.y = y;
		this.mouse = mouse;
		this.editor = editor;
	...
	public void tick(int index){
		if(mouse != null){
			if( mouse.x > x && mouse.x < x + 32 && mouse.y > y && mouse.y < y + 32){
				if(mouse.left.down){
					setFilled(editor.selectImageByLayer(selectedLayer, index));
					this.index = index;
				}
				if(mouse.right.down){
					resetTile();
